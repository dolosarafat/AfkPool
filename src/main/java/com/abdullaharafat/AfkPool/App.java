...
public class App extends JavaPlugin {
    ...
    public void onEnable() {
        ...
        if (ess != null && Bukkit.getServer().getPluginManager().isPluginEnabled("WorldGuard")) {
            ...
            getLogger().severe("EssentialsX or WorldGuard not found or not enabled. Disabling AfkPool.");
                ...
                if (region != null
                    ...
                    if (region.getMembers().contains(player.getName())) {
                        ...
                        if (currentTime - moneyTime >= moneyInterval) {
                            int money = ThreadLocalRandom.current().nextInt(30, 101); // Generate a random amount
                                                                                      // between 30 and 100 (inclusive)
                            Bukkit.dispatchCommand(Bukkit.getConsoleSender(),
